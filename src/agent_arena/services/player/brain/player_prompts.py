...
# Few-shot strategy guides distilled per (motion, side) from prior high-margin
# wins on each motion. Kept as a stable per-side prefix so it remains
# compatible with Gemini implicit context caching. Motions with no entry get an
# empty playbook (bootstrap from a first sweep, then mine and refill).
_AI_AUTONOMY = "Autonomous AI agents should be allowed to make consequential decisions without human approval"

MOTION_PLAYBOOKS: dict[str, dict[str, str]] = {
    _AI_AUTONOMY: {
        "PRO": (
...
def get_side_playbook(motion: str, side: str) -> str:
    """Return the few-shot playbook for (motion, side), empty if not yet mined."""
    return MOTION_PLAYBOOKS.get(motion, {}).get(side, "")
...
def build_player_prompt(context: PlayerContext, config: Any) -> tuple[str, dict[str, Any]]:
    ...
    # Playbook is stable per (motion, side) so it preserves the cache key.
    side_playbook = get_side_playbook(motion, context.side)
    prompt_parts = [
        schema_str,
